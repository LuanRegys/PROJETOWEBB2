...
// --- Função para pegar gameId da URL ---
...
// --- Evento para mudança do dropdown ---
...
// Carrega lista de jogos e seleciona o jogo via gameId da URL, se houver
window.addEventListener('DOMContentLoaded', async () => {
  await loadPublicGames();

  const gameId = getGameIdFromUrl();
  if (gameId) {
    const option = Array.from(fileDropdown.options).find(opt => opt.value === gameId);
    if (option) {
      fileDropdown.value = gameId;
      loadGameData(gameId);
    } else {
      alert('Jogo não encontrado no catálogo.');
      clearGame();
    }
  } else {
    clearGame();
